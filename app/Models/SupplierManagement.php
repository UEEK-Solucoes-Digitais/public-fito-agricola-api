...
class SupplierManagement extends BaseModel
{
    ...
    {
        return $this->hasOne(BankAccountManagement::class, 'id', 'account_id')->with('bank');
    }

    public static function readSuppliers($admin_id, $filter, $page, $only_active = false)
    {
    ...
    public static function readSupplier($id, $admin_id = null)
    {
        $supplier = self::select('*')->where('status', '!=', 0)->with('account')->where('id', $id)->first();
